...
abort_no_files <- function(dir) {
  msg <- cli::format_inline("Error: no files in {.path {dir}} match that pattern.")
  abort(msg)
}
...
abort_no_cache <- function(dir) {
  msg <- cli::format_inline("Error: no caches in {.path {dir}} match that pattern.")
  abort(msg)
}

abort_mixed_types <- function() {
  msg <- cli::format_inline("Error: Ambiguous pattern with more than one file type.")
  abort(msg)
}

abort_unsupported_type <- function() {
  msg <- cli::format_inline("Error: The file type matching that pattern is not supported by zread.")
  abort(msg)
}
